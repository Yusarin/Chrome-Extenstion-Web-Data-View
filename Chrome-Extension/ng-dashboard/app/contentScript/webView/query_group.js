function modifyDOM() {
  ...
  */
  ...
  for(var id=0; id < elems.length; id++) {
    ...
    //     elems[id].tagName == 'H5' ||
    //     elems[id].tagName == 'H6')
    if (elems[id].text === undefined)
    { domSerial['text'] = ""; }
    else
    {  domSerial['text'] = elems[id].text; }
    ...
    var domSerialHash = [domSerial['bottom'], domSerial['left'], domSerial['top'], domSerial['right'], domSerial['tag'], domSerial['class']];
  ...
  for(var id=0; id<elems.length; id++) {
    ...
    parentHash = parentHash.concat(parent.tagName);
    parentHash = parentHash.concat(parent.className);
