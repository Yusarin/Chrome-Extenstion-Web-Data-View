    ...
    for node in all_nodes.keys():
        ...
        if ext_type == 'text':
            if match_text_length(all_nodes[node], min_length, max_length) and match_font_size(all_nodes[node], font_size) and match_class_name(all_nodes[node],class_name) and \
               match_str_contains(all_nodes[node],text_contains) and match_str_begins(all_nodes[node],text_begins) and match_str_regex(all_nodes[node],regex_string) and is_numericornot(all_nodes[node], isnumeric) \
               and match_tagname(all_nodes[node], tag_name) and match_boundingbox(all_nodes[node],boxwidth_min,boxwidth_max,boxheight_min,boxheight_max) and match_startpos(all_nodes[node],boxstartx_min ,boxstarty_min,
               boxstartx_max,boxstarty_max):
